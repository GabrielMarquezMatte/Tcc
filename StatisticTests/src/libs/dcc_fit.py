...
def CalculateAllCorrelations(returns: np.ndarray, volatilities: np.ndarray,
    ...
    log_likelihood = pi_part - 0.5 * (np.log(np.linalg.det(unconditional_cov)) + returns[0, :].T @ np.linalg.inv(unconditional_cov) @ returns[0, :])
    for t in range(1, n):
...
def FitDcc(returns: np.ndarray, arch_results: list[ARCHModelResult]):
    ...
    conds = {'type': 'ineq', 'fun': lambda x: np.array([1 - x[0] - x[1]])}
    bounds = [(0, 1), (0, 1)]
...
def main():
    ...
    values = yf.download(tickers, start="2010-01-01", end="2023-01-01")['Adj Close']
